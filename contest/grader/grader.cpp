...
public:
	...
	void run_next_test() {
		if(test_idx_ == test_files_.size()) return finish();
		...
		auto S = [&]() -> td::Status {
			...
			if(!block::gen::t_Block.cell_unpack(root, rec)) return td::Status::Error("Failed to parse block root");
			vm::CellSlice cs{rec.state_update->load_cell().move_as_ok()};
			if(cs.special_type() != vm::CellTraits::SpecialType::MerkleUpdate) return td::Status::Error("Invalid Merkle Update in block");
			original_merkle_update = rec.state_update;
			rec.state_update = vm::CellBuilder{}.finalize_novm();
			if(!block::gen::pack_cell(root, rec)) return td::Status::Error("Failed to pack new block root");
